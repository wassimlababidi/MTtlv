...
void main()
{
    ...
    s=createHEADER( &version , macPresent,0,
                   tstmpVer,  typeVer,
                   lenPresent, &MTNtlv);
    if (s==0)
    {
...
/*
 inserts object in buffer till maximum size is reached then prints , can take as optional arguements N in case of multi-tlv mac, timestamp and length.
 mac always preceeds the other 2 , and timestamp always should preceed the length, N is always first
 */

{
    ...
    else
    {
        ...
        *offset+=1;
        if (HEADER_GET_TYPE(MTNtlv->header)==1)
        {
            buffer[*offset]=va_arg(ap,uint8_t);
        }
